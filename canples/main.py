...
import pickle
filename = 'canples/models/cpl_roster_classifier.sav'
...
def load_main_files(year):
    results = pd.read_csv(f'canples/datasets/{year}/cpl-{year}-results.csv') # get current year results
    if year == '2019':
...
def load_secondary_files(year):
    stats = pd.read_csv(f'canples/datasets/{year}/cpl-{year}-stats.csv') # load player statistics
    current_teams = pd.read_csv('canples/datasets/teams.csv') # get the current team information
...
@canples.route('/teams')
def teams():
    current_teams = pd.read_csv('canples/datasets/teams.csv')
    if year == '2019':
